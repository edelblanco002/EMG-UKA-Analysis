...
---------
   ...
   .. function:: gatherDataIntoTable.buildTable(dirPath,utteranceFiles,rowSize[,tableFileName='table'])
   ...
   :param rowSize: Size of each row of the table. It includes the features and the label, so the size of the row is the number of features + 1.
   :type rowSize: int
   ...
   Example of use::
      ...
      >>> uttType='whispered'
      >>> subset = 'test'
      >>> utteranceFiles = gatherDataIntoTable.getFilesList(dirpath,uttType)
      >>> utteranceFiles
      ...
      >>> rowSize = 931 # 6 channels, 5 features, 15 frames stacked after and before, 1 label
      >>> filename = 'newTable'
      >>> gatherDataIntoTable.buildTable(dirpath, utteranceFiles, rowSize, tableFileName=filename)
